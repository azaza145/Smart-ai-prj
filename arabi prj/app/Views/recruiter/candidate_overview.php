...
if (!$hasValidCv && !empty($cvs)) {
    foreach ($cvs as $cv) {
        if (!empty($cv['file_path']) && \App\Models\Cv::resolveFullPath($basePathForCv, $cv['file_path'])) {
            $hasValidCv = true;
            $pdfUrl = '/recruiter/candidates/' . (int)$c['id'] . '/cv/' . (int)$cv['id'];
            $latestCv = $cv;
            break;
        }
    }
...
$cvDisplayUrl = $hasValidCv ? $pdfUrl : $cvProfilUrl;
?>

<!-- Même design que détail candidat : profil (cartes) à gauche pleine largeur, CV à droite -->
<div class="candidate-overview-grid" style="display:grid;grid-template-columns:minmax(0,1fr) 420px;gap:28px;align-items:start;margin-bottom:24px;">
  <!-- LEFT: Profil en cartes (pleine largeur) -->
  <div id="cv-content" style="min-width:0;">
    <div class="candidate-detail-hero" style="background:<?= htmlspecialchars($avatarColor) ?>;padding:32px 40px;color:#fff;border-radius:16px 16px 0 0;box-shadow:0 4px 20px rgba(0,0,0,0.12);">
      <div style="display:flex;flex-wrap:wrap;align-items:center;gap:24px;justify-content:space-between;">
        <div style="display:flex;align-items:center;gap:24px;">
          ...
          <div>
            <h1 style="font-size:28px;font-weight:700;margin:0 0 6px 0;letter-spacing:-0.02em;"><?= htmlspecialchars($fullName !== '' ? $fullName : '—') ?></h1>
        ...
        <div style="display:flex;flex-direction:column;align-items:flex-end;gap:12px;">
          <div style="display:flex;flex-wrap:wrap;gap:10px;justify-content:flex-end;">
            ...
            <form method="post" action="/recruiter/candidates/<?= (int)$c['id'] ?>/fill-from-cv" style="display:inline;">
              ...
              <button type="submit" class="btn" style="background:rgba(255,255,255,0.25);color:#fff;border:2px solid rgba(255,255,255,0.5);padding:10px 20px;font-size:13px;font-weight:600;border-radius:10px;cursor:pointer;" title="Réextraire expérience, formation, compétences depuis le PDF (Ollama)">🔄 Re-extraire le CV</button>
            </form>
            <?php } ?>
    ...
    <div style="margin-top:0;">
      ...
      <div class="candidate-detail-card" style="background:#fff;border-radius:16px;box-shadow:0 4px 20px rgba(0,0,0,0.08);border:1px solid var(--border);margin-top:20px;overflow:hidden;">
        <div style="background:var(--ca-l);padding:14px 24px;display:flex;align-items:center;gap:12px;">
          <span style="font-size:22px;">🎓</span>
          <h2 style="font-size:13px;font-weight:700;letter-spacing:1px;text-transform:uppercase;color:var(--g-dark);margin:0;">Formation</h2>
        </div>
        <div style="padding:24px;">
          <?php if (count($education) > 0) { foreach ($education as $idx => $ed) {
            $deg = trim($ed['degree'] ?? ''); $school = trim($ed['school'] ?? ''); $year = trim($ed['year'] ?? '');
          ?>
          <div style="padding:10px 0;<?= $idx > 0 ? 'border-top:1px solid var(--border);' : '' ?>">
            <div style="font-weight:700;font-size:14px;"><?= htmlspecialchars($deg) ?></div>
            <div style="font-size:13px;color:var(--ca);"><?= htmlspecialchars($school) ?><?= $year !== '' ? ' · ' . htmlspecialchars($year) : '' ?></div>
          </div>
          <?php } } else if (!empty($c['diplome']) || !empty($c['universite'])) { ?>
          <div style="font-weight:700;font-size:14px;"><?= htmlspecialchars(trim(($c['diplome'] ?? '') . ' ' . ($c['education_niveau'] ?? ''))) ?: '—' ?></div>
          <div style="font-size:13px;color:var(--ca);"><?= htmlspecialchars($c['universite'] ?? '') ?></div>
          <?php } else { ?><div style="color:var(--muted);font-size:14px;"><?= htmlspecialchars($emptyPh) ?></div><?php } ?>
        </div>
      </div>
      ...
      <div class="candidate-detail-card" style="background:#fff;border-radius:16px;box-shadow:0 4px 20px rgba(0,0,0,0.08);border:1px solid var(--border);margin-top:20px;overflow:hidden;">
        <div style="background:var(--ca-l);padding:14px 24px;display:flex;align-items:center;gap:12px;">
          ...
          <h2 style="font-size:13px;font-weight:700;letter-spacing:1px;text-transform:uppercase;color:var(--g-dark);margin:0;">Compétences</h2>
        </div>
        <div style="padding:24px;">
          <?php if (count($skills) > 0) { ?>
          <div style="display:flex;flex-wrap:wrap;gap:8px;">
            <?php foreach ($skills as $skill) { ?><span style="background:var(--ca-l);color:var(--g-dark);padding:6px 14px;border-radius:8px;font-size:13px;font-weight:600;"><?= htmlspecialchars($skill) ?></span><?php } ?>
          </div>
          <?php } else { ?><div style="color:var(--muted);font-size:14px;"><?= htmlspecialchars($emptyPh) ?></div><?php } ?>
        </div>
      </div>
      ...
      <div class="candidate-detail-card" style="background:#fff;border-radius:16px;box-shadow:0 4px 20px rgba(0,0,0,0.08);border:1px solid var(--border);margin-top:20px;overflow:hidden;">
        ...
        <div style="padding:24px;">
          ...
          <div style="display:flex;flex-direction:column;gap:14px;">
            ...
            <div><div style="font-size:14px;font-weight:600;margin-bottom:6px;"><?= htmlspecialchars($lang) ?></div><div style="height:8px;background:var(--border);border-radius:4px;overflow:hidden;"><div style="height:100%;width:85%;background:linear-gradient(90deg,var(--ca),var(--g-mid));border-radius:4px;"></div></div></div>
            <?php } ?>
          </div>
          <?php } else { ?><div style="color:var(--muted);font-size:14px;"><?= htmlspecialchars($emptyPh) ?></div><?php } ?>
        </div>
      ...
      <div class="candidate-detail-card" style="background:#fff;border-radius:16px;box-shadow:0 4px 20px rgba(0,0,0,0.08);border:1px solid var(--border);margin-top:20px;overflow:hidden;">
        ...
        <div style="padding:24px;"><ul style="margin:0;padding-left:20px;"><?php foreach ($certifications as $cert) { ?><li style="font-size:14px;margin-bottom:6px;"><?= htmlspecialchars($cert) ?></li><?php } ?></ul></div>
      </div>
      <?php } ?>
...
<style>
  .candidate-overview-grid .candidate-detail-hero { transition: box-shadow .2s ease; }
  .candidate-overview-grid .candidate-detail-hero:hover { box-shadow: 0 8px 28px rgba(0,0,0,0.15); }
  .candidate-overview-grid .candidate-detail-card { transition: transform .15s ease, box-shadow .2s ease; }
  .candidate-overview-grid .candidate-detail-card:hover { transform: translateY(-2px); box-shadow: 0 8px 24px rgba(0,0,0,0.1); }
  @media (max-width: 1024px) {
    .candidate-overview-grid { grid-template-columns: 1fr !important; }
    .candidate-overview-grid .card { position: static !important; }
  }
</style>
<style media="print">
  @media print {
    .topnav, .btn, a[href], form, .card-header, #cv-content, .candidate-overview-grid > div:first-child { display: none !important; }
